...
const convertToRows = originColumns => {
  ...
  //     console.log("2222", column.rowSpan);
  //   } else {
  //     console.log("33333");
  //     column.rowSpan = 1;
  //   }
  ...
  let colRowIndex = 0;
  originColumns.forEach(column => {
    colRow[colRowIndex][0] = column;
    colRowIndex++;
  });
  return { colRow: colRow, lastData: originColumns };
};
...
const headersToRows = originColumns => {
  ...
  //   // }
  //   // if (column.children) {
  //   //   console.error("333333333333333333333333333");
  //   //   let colSpan = 0;
  //   //   column.children.forEach((subColumn) => {
  //   //     //进行递归
  //   //     traverse(subColumn, column);
  ...
  //   // rows[column.level - 1].push(column);
  // });
  rows[0] = originColumns;
  // console.error(rows, "rowsrowsrowsrowsrowsrowsrowsrowsrowsrowsrows");
  return { rows: rows, lastData: originColumns };
};
  ...
  computed: {
    ...
    actualHeight() {
      ...
      if (this.colHead.length > 0) {
        ...
        height += this.lastDataCol.length * (this.tableTdHeight + 20 / this.lastDataCol.length);
      }
    ...
    tableData() {
      ...
      if (this.headers.length > 0 && this.colHead.length === 0) {
        ...
        this.tableValue.forEach(item => {
          arr[item.src_device_id + "_" + item.dst_device_id] = item;
        });
        // console.error(arr, "arrarrarrarrarrarrarr");
      }
